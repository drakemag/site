...
if (PhpCaptcha::Validate($_POST['user_code'])) {
	...
	if ($writeToDB) {
		...
		if ($include_date_submit) {
			...
			$tmpStr = $_POST[$email_field_name] . "\n";
		}
	    $numBytes = fwrite($fp, $tmpStr);
		fclose($fp);
	}

$admin_email = "dawn@drakemag.com,info@drakemag.com"; //The email address you want the POSTDATA sent to.
$emailSubject = "Drake Magazine - Contact Request"; //Subject of the emails sent by this script.
$timeOfDay = date("Y:m:d H:i"); //Format of the time displayed in the email. Refer to the PHP date() documentation for details on changing it.
				...
				$mailMessage = 
					...
					//"county: " . $_POST[$fields_county] . "\r\n" . 
					"Message: " . $_POST[$fields_comments] . "\r\n";
					"Learn: " . $_POST[$fields_learn] . "\r\n" .
