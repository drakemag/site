...
if (!file_exists(JPATH_SITE.'/administrator/components/com_storelocator/')) {
	echo "<div style=\"border:1px solid #c30;color:#c30;margin: 0 0 5px 0;padding: 5px;font-weight: bold;text-align:left;text-align:center;\">The Store Locator component is not installed.</div>"; 
	return;
}
...
$query = "SELECT id as value, name as text FROM #__storelocator_cats ORDER BY text Asc";
$db->setQuery($query);
$catResult = $db->loadAssocList();

array_unshift($catResult, (object)array('value'=>-1,'text'=>JText::_('MOD_STORELOCATOR_SEARCH_ALL_CATEGORIES')));
$catsearch = JHTML::_('select.genericlist',   $catResult, 'catid', 'class="input-medium" size="1"', 'value', 'text');

// Feature Filter
$featstate = array();
$featstate[] = (object)array('value'=>0,'text'=>JText::_('MOD_STORELOCATOR_SEARCH_ALL_LOCATIONS'));
$featstate[] = (object)array('value'=>1,'text'=>JText::_('MOD_STORELOCATOR_SEARCH_ONLY_FEATURED'));
$featsearch = JHTML::_('select.genericlist',   $featstate, 'featstate', ' class="input-medium" size="1"', 'value', 'text', 0);
...
if ( empty($search_text) || $search_text == '' || !isset($search_text) ) {
	$search_text = JText::_('MOD_STORELOCATOR_SEARCH_SEARCH_LOCATIONS');
} 
...
<fieldset>
  <label><?php echo JText::_( 'MOD_STORELOCATOR_SEARCH_ADDRESS' ); ?></label>
  <input type="text" id="sl_search_address" name="mod_addressInput" placeholder="<?php echo $search_text?>" class="input-medium" />
   
  <label><?php echo JText::_( 'MOD_STORELOCATOR_SEARCH_RADIUS' ); ?></label>
  <select id="sl_search_radiusSelect" name="radiusselect"  class="input-medium">
      	<?php
			...
			printf("<option value=\"%d\" %s>%d %s</option>",
					...
					$radius,
		 			($map_units?JText::_( 'MOD_STORELOCATOR_SEARCH_MILES' ):JText::_( 'MOD_STORELOCATOR_SEARCH_KILOMETERS' ))
					);
		?>
      </select>
      
       <?php if($featsearch_enabled) : ?>
       		<label><?php echo JText::_('MOD_STORELOCATOR_SEARCH_FEATURED'); ?></label> 
           <?php echo $featsearch; ?>
       <?php endif; ?>
       <?php if($catsearch_enabled) : ?>
           <label><?php echo JText::_('MOD_STORELOCATOR_SEARCH_CATEGORY'); ?></label> 
		   <?php echo $catsearch; ?>
       <?php endif; ?>
      
  <input type="submit" class="btn btn-info" value="<?php echo JText::_( 'MOD_STORELOCATOR_SEARCH_SEARCH' ); ?>"/>
