...
class CommunityPollsModelPolls extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		if (APP_VERSION >= 3 && JLanguageAssociations::isEnabled())
		{
			$query->select('COUNT(asso2.id)>1 as association')
				->join('LEFT', '#__associations AS asso ON asso.id = a.id AND asso.context=' . $db->quote('com_communitypolls.item'))
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					...
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_communitypolls.poll')
				);
		}
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__jcp_polls AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
