...
class SPTransferModelLog extends JModelList
{
	...
	public function delete() {
        ...
        $pks = (array) $pks;
        $table = CYENDFactory::getTable('Log', 'SPTransferTable');
    ...
    public function get_last_id() {
        ...
        $query = $db->getQuery(true);
        $query->select('*')
                ->from('#__sptransfer_log AS a');
        $query->select('ta.extension_name AS extension_name, ta.name AS name')
			->join('LEFT', '#__sptransfer_tables AS ta ON ta.id = a.tables_id');
        ...
        $message = '<p><b>Last Processed Item</b><p>' .
                '<p>' .
                '<b>Type: </b>' . $result['extension_name'] . ' -> ' . $result['name'] .
                '<br/>' . 
                '<b>ID: </b>' . $result['source_id'] .
                '<br/>' . 
                '<b>Update time: </b>' . $result['created'] .
                '</p>';
