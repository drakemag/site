 ...
 */
class StorelocatorTablelocation extends JTable {
    ...
    public function __construct(&$db) {
        parent::__construct('#__storelocator_locations', 'id', $db);
    }
    ...
    public function bind($array, $ignore = '') {
		...
		//Support for multiple or not foreign key field: tags
			if(isset($array['tags'])):
				...
				else if(strrpos($array['tags'], ',') != false){
					$array['tags'] = explode(',',$array['tags']);
				}
			endif;
		$input = JFactory::getApplication()->input;
		$task = $input->getString('task', '');
		if(($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state','com_storelocator') && $array['state'] == 1)){
			$array['state'] = 0;
		}
        ...
        if(!JFactory::getUser()->authorise('core.admin', 'com_storelocator.location.'.$array['id'])){
            $actions = JFactory::getACL()->getActions('com_storelocator','location');
            $default_actions = JFactory::getACL()->getAssetRules('com_storelocator.location.'.$array['id'])->getData();
            $array_jaccess = array();
    ...
    */
    protected function _getAssetName() {
        $k = $this->_tbl_key;
        return 'com_storelocator.location.' . (int) $this->$k;
    }
