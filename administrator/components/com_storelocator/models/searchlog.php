 ...
 */
class StorelocatorModelsearchlog extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		parent::populateState('a.ipaddress', 'asc');
	}
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__storelocator_log_search` AS a');
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.ipaddress LIKE '.$search.' )');
			}
		}
	...
	public function delete()
	{
		...
		else
		{
			...
			foreach($ids as $id)
			{
				$query = 'DELETE FROM #__storelocator_log_search WHERE id = '.intval($id);
